...
let signUpFunction = (req, res) => {
                    ...
                    if (err) {
                        ...
                        newUser.save((err, newUser) => {
                            if (err) {
                                ...
                                let apiResponse = response.generate(false, 'User created successfully', 200, null);
                                res.send(apiResponse);
                                mailer.sendEmail(newUser.email, 'Issue Tracker',
                                    `
                                    
                                       <h5 title">Issue Tracker</h5>
                                 <p>Hello, Welcome to Issue tracker tool</p><br/>
                            Have a Great day,<br/>Issue Tracker team.` )
    ...
    validateUserInput(req, res)
        ...
        .catch((err) => {
            console.log(err);
            let apiResponse = response.generate(true, err.message, 500, err);
        ...
        return new Promise((resolve, reject) => {
            passwordLib.comparePassword(req.body.password, retrievedUserDetails.password, (err, isMatch) => {
                if (err) {
                    ...
                    let apiResponse = response.generate(true, 'Wrong Passwor, please type a correct one', 400, null)
                    reject(apiResponse)
                }
            })
        })
...
let forgotPassword = (req, res) => {
    UserModel.findOne({ email: req.body.email }, (err, result) => {
        if (err) {
            let apiResponse = response.generate(true, 'Some error ocurred , failed to look user', 500, null);
            res.send(apiResponse);
        } else if (check.isEmpty(result)) {
            ...
            mailer.sendEmail(result.email, 'Reset Password',
                `Hello User, <br/> 
            <a href = 'http://localhost:4000/api/v1/users/resetPassword/${result.userId}'>
            Click here to reset the password</a></br>
            Thank you , <br/> Issue Tracker`);
...
let resetPassword = (req, res) => {
    ...
    let updatePassword = (userDetails) => {
        return new Promise((resolve, reject) => {
            if (check.isEmpty(req.body.password)) {
                let apiResponse = response.generate(true, 'Bad request, password missing', 400, null);
                reject(apiResponse);
            } else {
                try {
                    UserModel.updateOne({ userId: req.body.userId },
                        ...
                        (err, result) => {
                            if (err) {
                                ...
                                mailer.sendEmail(userDetails.email, "Password reset",
                                    `Hello user,<br/><br/> 
                            Your login password for Issue Tracking Tool has been changed successfully.<br/<br/>br/>
                            Thank you,<br/>Issue Tracker.`);
                                let apiResponse = response.generate(false, 'Password changed', 200, null);
                                resolve(apiResponse);
                            }
                        });
                } catch (err) {
    ...
    findUser(req, res)
        ...
        .then((resolve) => {
            res.status(200);
            let apiResponse = response.generate(false, 'Pasword changed', 200, resolve);
            res.send(apiResponse);
        }).catch((err) => {
